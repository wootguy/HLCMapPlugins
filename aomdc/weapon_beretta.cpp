...
class CBeretta : public CWeaponCustom {

	void Spawn()
	{
		m_iDefaultAmmo = BERETTA_MAX_CLIP;
		m_iId = g_beretta_info.iId;
		CWeaponCustom::Spawn();
	}
	...
	void Precache() {
		m_defaultModelV = "models/aomdc/v_beretta.mdl";
		m_defaultModelP = "models/aomdc/p_beretta.mdl";
		m_defaultModelW = "models/aomdc/w_beretta.mdl";
		CBasePlayerWeapon::Precache();

		int shootSnd = PRECACHE_SOUND("aomdc/weapons/ber_fire.wav");
		...
		//params.idles[1] = { BERETTA_LONGIDLE, 1, 1730 };
		...
		AddEvent(WepEvt().PrimaryNotEmpty().WepAnim(BERETTA_FIRE1).AddAnim(BERETTA_FIRE2));
		AddEvent(WepEvt().PrimaryEmpty().WepAnim(BERETTA_FIRE_LAST));
	...
	void GetAmmoDropInfo(bool secondary, const char*& ammoEntName, int& dropAmount) {
		ammoEntName = "ammo_beretta";
		dropAmount = BERETTA_MAX_CLIP;
	}
};
