...
Vector rotatePoint(Vector pos, Vector angles)
{
	...
	vector<float> yawRotMat = rotationMatrix(yawAxis, angles.y * (M_PI / 180.0f));
	pitAxis = matMultVector(yawRotMat, pitAxis);
	rollAxis = matMultVector(yawRotMat, rollAxis);

	vector<float> pitRotMat = rotationMatrix(pitAxis, angles.x * (M_PI / 180.0f));
	rollAxis = matMultVector(pitRotMat, rollAxis);

	vector<float> rollRotMat = rotationMatrix(rollAxis, angles.z * (M_PI / 180.0f));
