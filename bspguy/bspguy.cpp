...
namespace bspguy {
	...
	void delay_fire_targets(string target) {
		...
		FireTargets(target, NULL, NULL, USE_TOGGLE);
	}
	...
	void mapchange_internal(string thisMap, string nextMap, bool fastchange=false) {
		...
		UTIL_ClientPrintAll(HUD_PRINTCENTER, "Entering " + nextMap + "\n");
		
		float extraDelay = fastchange ? 0.0f : 1.5f;
		
		if (thisMap != nextMap) {
			vector<string>* levelKeys = map_loaded.getKeys();
	...
	void mapchange(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float flValue)
	{
		...
		if (thisMap == "" || nextMap == "") {
			println("[bspguy] ERROR: bspguy_mapchange called by " + pCaller->pev->classname + " which is missing $s_bspguy_map_source || $s_next_map");
			return;
		}
	...
	{		
		...
		if (cleanMap.Length() > 0) {
			vector<string> maps = cleanMap.Split("+");
		...
		if (trigger.Length() > 0) {
			...
			if (triggerTypeSep != -1) {
				...
				if (triggerType == 0) {
					...
					do {
						*ent = UTIL_FindEntityByTargetname(ent, trigger);
						if (ent ) {
							UTIL_Remove(ent);
						}
					} while (ent );
			...
			if (!killed)
				FireTargets(trigger, pActivator, pCaller, useType);
		}
	}
	...
	void loadMapEnts() {
		string entFileName = string(gpGlobals->mapname).ToLowercase() + ".ent";
	...
	bool isSpawnEntity(string cname) {
		return cname == "info_player_deathmatch" || cname == "info_player_start"
				or cname == "info_player_dm2" || cname == "info_player_coop";
	}
	
	void deleteMapEnts(string mapName, bool invertFilter, bool spawnsOnly) {
		mapName = mapName.ToLowercase();
		string infoEntName = "bspguy_info_" + mapName;
		CBaseEntity* mapchangeEnt = UTIL_FindEntityByTargetname(NULL, infoEntName);
		...
		if (mapchangeEnt ) {
			...
			println("[bspguy] ERROR: Missing entity '" + infoEntName + "'. Some entities may not be deleted in previous maps, && that can cause lag!");
		}
		...
		do {
			*ent = g_EntityFuncs.FindEntityByClassname(ent, "*");
			if (ent ) {
				...
				if (ent.IsPlayer() || string(ent->pev->targetname).Find("bspguy") == 0) {
					continue;
				}
				
				// don't remove player items/weapons
				CBasePlayerItem* item = cast<CBasePlayerItem*>(ent);
				if (item  && item.m_hPlayer.IsValid()) {
					continue;
				}
				
				KeyValueBuffer* pKeyvalues = g_engfuncs.pfnGetInfoKeyBuffer( ent->edict() );
	...
	void spawnMapEnts(string mapName) {
		...
		for (uint i = 0; i < g_ent_defs.size(); i++) {
			...
			if (mapSource == mapName) {
				...
				CBaseEntity* ent = CreateEntity(classname, g_ent_defs[i], true);
				
				if (ent  && string(ent->pev->classname) == "func_train") {
		...
		FireTargets("bspguy_init_" + mapName, NULL, NULL, USE_TOGGLE);
	}
	...
	void survival_restart_check() {
		...
		UTIL_ClientPrintAll(HUD_PRINTTALK, "This is a merged map. The current section will reload shortly.\n");	
	...
	void printMapSections(EHANDLE h_plr) {
		...
		UTIL_ClientPrint(plr, HUD_PRINTCONSOLE, "Map sections:\n");	
		for (uint i = 0; i < map_order.size(); i++) {
			string begin = i < 9 ? "     " : "    ";
			...
			UTIL_ClientPrint(plr, HUD_PRINTCONSOLE, begin + (i+1) + ") " +  map_order[i] + end);
		}
	}
	...
	void doCommand(CBasePlayer* plr, const CCommand* args, bool inConsole) {
		...
		if (args.ArgC() >= 2)
		{
			...
			UTIL_ClientPrint(plr, HUD_PRINTCONSOLE, '----------------------------------bspguy commands----------------------------------\n\n');
			UTIL_ClientPrint(plr, HUD_PRINTCONSOLE, 'Type "bspguy list" to list map sections.\n\n');
			UTIL_ClientPrint(plr, HUD_PRINTCONSOLE, 'Type "bspguy spawn" to test spawn points.\n\n');
			UTIL_ClientPrint(plr, HUD_PRINTCONSOLE, 'Type "bspguy mapchange [name|number]" to transition to a new map section.\n');
			UTIL_ClientPrint(plr, HUD_PRINTCONSOLE, '    [name|number] = Optional. Map section name || number to load (as shown in "bspguy list")\n');
			UTIL_ClientPrint(plr, HUD_PRINTCONSOLE, '\n-----------------------------------------------------------------------------------\n\n');
		}
	}
