...
void print(string text) { ALERT( at_console, text); }
...
// water splashes && bubble trails for bullets
void water_bullet_effects(Vector vecSrc, Vector vecEnd)
{
	// bubble trails
	bool startInWater = g_engfuncs.pfnPointContents(vecSrc) == CONTENTS_WATER;
	bool endInWater = g_engfuncs.pfnPointContents(vecEnd) == CONTENTS_WATER;
...
vector<float> eulerMatrix(Vector angles)
{
...
vector<float> invertMatrix(vector<float> m)
{
	vector<float> inv(16);
...
{		
	...
	for (uint i = 0; i < dirs.length(); i++)
	{
		...
		TRACE_LINE( src, end, ignore_monsters, edict, tr );
		...
		if (tr.flFraction < 1.0 )
		{
			...
			if (tr.pHit )
			{
				decalTarget.ent = EHANDLE(CBaseEntity::Instance( tr.pHit ));
...
Vector rotatePoint(Vector pos, Vector angles)
{
	...
	vector<float> yawRotMat = rotationMatrix(yawAxis, Math.DegreesToRadians(angles.y));
	pitAxis = matMultVector(yawRotMat, pitAxis);
	rollAxis = matMultVector(yawRotMat, rollAxis);
	
	vector<float> pitRotMat = rotationMatrix(pitAxis, Math.DegreesToRadians(angles.x));
	rollAxis = matMultVector(pitRotMat, rollAxis);
	
	vector<float> rollRotMat = rotationMatrix(rollAxis, Math.DegreesToRadians(angles.z));
...
Vector unwindPoint(Vector pos, Vector angles)
{
	...
	vector<float> yawRotMat = rotationMatrix(yawAxis, angles.y);
	pitAxis = matMultVector(yawRotMat, pitAxis);
	rollAxis = matMultVector(yawRotMat, rollAxis);
	vector<float> pitRotMat = rotationMatrix(pitAxis, angles.x);
	rollAxis = matMultVector(pitRotMat, rollAxis);
	...
	vector<float> rollRotMat = rotationMatrix(rollAxis, -angles.z);
...
class WeaponCustomProjectile : ScriptBaseAnimating
{
	...
	void MoveThink()
	{
		...
		if (shoot_opts.effect4.valid && nextTrailEffectTime < gpGlobals->time)
		{
			nextTrailEffectTime = gpGlobals->time + options.trail_effect_freq;
			CBaseEntity* owner = CBaseEntity::Instance( pev->owner );
			EHANDLE howner = owner;
			custom_effect(pev->origin, shoot_opts.effect4, EHANDLE(self), howner, howner, pev.velocity.Normalize(), shoot_opts.friendly_fire ? 1 : 0);
		}
		...
		if (weaponPickup)
		{
			if (pev.velocity.Length() < 128)
			{
				...
				do {
					*ent = UTIL_FindEntityInSphere(ent, pev.origin, pickupRadius, "player", "classname"); 
					if (ent )
					{
						CBasePlayer* plr = (CBasePlayer*)(ent);
						plr.SetItemPickupTimes(0);
						if (plr.HasNamedPlayerItem(pickup_classname) )
						{
	...
	{	
		if (ent  == NULL  || ent->entindex() == 0 || shoot_opts.shoot_type == SHOOT_MELEE)
			return;
		CBaseEntity* owner = CBaseEntity::Instance( pev->owner );
		...
		if ( tr.flFraction >= 1.0 || pHit->entindex() != ent->entindex())
		{
			...
			g_Utility.TraceHull( vecSrc, vecEnd, dont_ignore_monsters, head_hull, edict(), tr );
			if ( tr.flFraction < 1.0 )
			{
				...
				*pHit = CBaseEntity::Instance( tr.pHit );
				if ( pHit  == NULL  || pHit->IsBSPModel() )
					g_Utility.FindHullIntersection( vecSrc, tr, tr, VEC_DUCK_HULL_MIN, VEC_DUCK_HULL_MAX, edict() );
	...
	void Touch( CBaseEntity* pOther )
	{
		...
		if (event != PROJ_ACT_BOUNCE || nextBounceEffect < gpGlobals->time)
		{
			...
			custom_effect(pev->origin, effect, EHANDLE(self), htarget, howner, Vector(0,0,0), shoot_opts.friendly_fire ? 1 : 0);
		}
...
{
	return RANDOM_LONG(100-variance, 100+variance);
}
					...
					EHANDLE owner, Vector vDir, int flags, DecalTarget* dt=NULL)
{
	...
	Vector vel = delayFinished || !creator.IsValid() ? Vector(0,0,0) : creator.GetEntity()->pev->velocity;
	bool inWater = g_engfuncs.pfnPointContents(pos) == CONTENTS_WATER;
	...
	if (effect.spray_count > 0 && effect.spray_sprite.Length() > 0)
	{
	...
	if (effect.explode_gibs > 0 && effect.explode_gib_mdl.Length() > 0)
	{		
					  ...
					  CBaseEntity* decalEnt, EHANDLE owner, bool inWater, bool friendlyFire)
{
	...
	if (inWater && effect.explode_water_spr.Length() > 0)
			...
			do {
				*victim = UTIL_FindEntityByClassname(victim, "player");
				if (victim )
				{
					...
					oldClassify.push_back(victim.GetClassification(0));
...
void RadiusDamage( Vector vecSrc, entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, float flRadius, int iClassIgnore, int bitsDamageType )
{
	...
	while ((*pEntity = UTIL_FindEntityInSphere( pEntity, vecSrc, flRadius, "*", "classname" )) != NULL)
	{
		...
		if ( pEntity->pev->takedamage != DAMAGE_NO )
		{
			...
			TRACE_LINE( vecSrc, vecSpot, dont_ignore_monsters, CBaseEntity::Instance(pevInflictor)->edict(), tr );
	...
	for (uint i = 0; i < sphereDirs.size(); i++)
	{
		...
		TRACE_LINE( vecSrc, vecSrc + sphereDirs[i]*flRadius, dont_ignore_monsters, CBaseEntity::Instance(pevInflictor)->edict(), tr );
...
void animate_view_angles(EHANDLE h_plr, Vector start_angle, Vector add_angle, float startTime, float endTime)
{
	...
	float progress = 1;
	if (totalTime > 0)
		progress = (gpGlobals->time - startTime) / totalTime;
	if (progress > 1)
		progress = 1;
...
{
	if (effect  == NULL  || !h_plr)
		return;
		
	CBaseEntity* plrEnt = h_plr;
	CBaseEntity* wepEnt = h_wep;
	CBasePlayer* plr = (CBasePlayer*)(plrEnt);
	...
	// push 'em
	MAKE_VECTORS( plrEnt->pev->v_angle );
	...
	// indicate something
	if (effect.action_sprite.Length() > 0 && plrEnt.IsPlayer())
	...
	{
		if (effect.v_model.Length() > 0 || effect.p_model.Length() > 0 || effect.w_model.Length() > 0 || effect.w_model_body >= 0)
		{
	...
	if (effect.hud_text.Length() > 0 && plrEnt.IsPlayer())
	{
	...
	if (effect.player_sprite_count > 0 && effect.player_sprite.Length() > 0 && plrEnt.IsPlayer())
	{
	...
	{
		// Remember old glow setting in case user is normally glowing due to map logic || server script
	...
	if (targetStr.Length() > 0)
	{
		...
		FireTargets(targetStr, plrEnt, effectEnt, USE_TYPE(effect.triggerstate));
	}
...
{
	if (ent.IsValid() && toucher.IsValid())
...
void WaterSoundEffects(Vector pos, SoundArgs* args)
{
	if (!args.underwaterEffects)
		return;
	if (g_engfuncs.pfnPointContents(pos) == CONTENTS_WATER)
	{
		args.pitch = Math.max(0, args.pitch - RANDOM_LONG(20, 30));
		args.volume *= 0.7f;
	}
}
...
void playSoundDelay(SoundArgs* args)
{
	if (args.attachToEnt)
	{
		...
		EMIT_SOUND_DYN( ent->edict(), args.channel, args.file, args.volume, args.attn, args.flags, 
									args.pitch, args.target_ent );
	}
	else
	{
		WaterSoundEffects(args.pos, *args);
		EMIT_SOUND_DYN( NULL, args.channel, args.file, args.volume, 
...
class WeaponSound
{
			  ...
			  int additionalFlags=0, bool userOnly=false)
	{
		...
		{
			WaterSoundEffects(ent->pev->origin + ent->pev->view_ofs, *args);
			EMIT_SOUND_DYN( ent->edict(), args.channel, args.file, args.volume, 
	...
	int getPitch()
	{
		if (options )
		{
			...
			return options->pev->rendermode + RANDOM_LONG(-pitch_rand, pitch_rand);
		}
		return 100;
	}
	...
	void stop(CBaseEntity* ent, SOUND_CHANNEL channel=CHAN_STATIC)
	{
		if (file.Length() <= 0)
			return;
		if (options  && options->pev->sequence > -1)
			channel = SOUND_CHANNEL(options->pev->sequence);
		g_SoundSystem.StopSound(ent->edict(), channel, file);
	}
}

vector<WeaponSound> parseSounds(string val)
{
	vector<string> strings = val.Split(";");
	vector<WeaponSound> sounds;
...
bool isBreakableEntity(CBaseEntity* ent)
{
	if (ent->pev->classname == "func_breakable")
		return true;
	if (ent->pev->classname == "func_door" || ent->pev->classname == "func_door_rotating")
...
bool shouldHealTarget(CBaseEntity* target, CBaseEntity* healer, weapon_custom_shoot* shoot_opts)
{
	...
	if (target->IsBSPModel() && isRepairable(target) && (healBreakables || healAll))
		return true;
...
bool shouldAttack(CBaseEntity* target, CBaseEntity* plr, weapon_custom_shoot* shoot_opts)
{
	...
	if (!shouldHealTarget(target, plr, shoot_opts) && !shoot_on_damage)
		return false;
	return true;
}
...
CBaseEntity* getReviveTarget(Vector center, float radius, CBaseEntity* healer, weapon_custom_shoot* shoot_opts)
{
	CBaseEntity* ent = NULL;
	do {
		*ent = UTIL_FindEntityInSphere(ent, center, radius, "*", "classname"); 
		if (ent )
		{
...
void projectile_follow_aim(EHANDLE h_plr, EHANDLE h_proj, weapon_custom_shoot* opts, float timeLeft)
{
	...
	TRACE_LINE( vecSrc, vecEnd, dont_ignore_monsters, proj->edict(), tr );
	...
	{
		CBaseEntity* enemy = CBaseEntity::Instance( proj->pev->enemy );
		CBaseEntity* ent = NULL;
		float bestDot = 0;
		...
		{
			do {
				*ent = UTIL_FindEntityInSphere(ent, proj->pev->origin, radius, "*", "classname"); 
				if (ent  && ent.IsMonster() && ent->IsAlive())
				{
			...
			*enemy = CBaseEntity::Instance( proj->pev->enemy );
		}
	...
	if (abs(angle) > 0.001f)
	{
		...
		g_engfuncs.pfnVecToAngles(proj->pev->velocity, proj->pev->angles);	
	}
...
void loadSoundSettings(WeaponSound* snd)
{
	...
	CBaseEntity* ent = UTIL_FindEntityByTargetname(NULL, snd.file);
	if (ent )
	{
...
weapon_custom_effect* loadEffectSettings(weapon_custom_effect* effect, string name="")
{
	...
	string searchStr = effect  ? effect.name : name;
	CBaseEntity* ent = UTIL_FindEntityByTargetname(NULL, searchStr);
	if (ent )
	{
...
weapon_custom_user_effect* loadUserEffectSettings(weapon_custom_user_effect* effect, string name="")
{
	...
	CBaseEntity* ent = UTIL_FindEntityByTargetname(NULL, name);
	if (ent )
	{
...
void loadSoundSettings(vector<WeaponSound>* sound_list)
{
...
void te_explosion(Vector pos, string sprite="sprites/zerogxplode.spr", int scale=10, int frameRate=15, int flags=0, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_EXPLOSION);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_SHORT(g_engfuncs.pfnModelIndex(sprite));m.WRITE_BYTE(scale);m.WRITE_BYTE(frameRate);m.WRITE_BYTE(flags);m.MESSAGE_END(); }
void te_smoke(Vector pos, string sprite="sprites/steam1.spr", int scale=10, int frameRate=15, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_SMOKE);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_SHORT(g_engfuncs.pfnModelIndex(sprite));m.WRITE_BYTE(scale);m.WRITE_BYTE(frameRate);m.MESSAGE_END(); }
void _te_beamcircle(Vector pos, float radius, string sprite, uint8 startFrame, uint8 frameRate, uint8 life, uint8 width, uint8 noise, Color c, uint8 scrollSpeed, NetworkMessageDest msgType, edict_t* dest, int beamType) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(beamType);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z + radius);m.WRITE_SHORT(g_engfuncs.pfnModelIndex(sprite));m.WRITE_BYTE(startFrame);m.WRITE_BYTE(frameRate);m.WRITE_BYTE(life);m.WRITE_BYTE(width);m.WRITE_BYTE(noise);m.WRITE_BYTE(c.r);m.WRITE_BYTE(c.g);m.WRITE_BYTE(c.b);m.WRITE_BYTE(c.a);m.WRITE_BYTE(scrollSpeed);m.MESSAGE_END(); }
...
void te_breakmodel(Vector pos, Vector size, Vector velocity, uint8 speedNoise=16, string model="models/hgibs.mdl", uint8 count=8, uint8 life=0, uint8 flags=20, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_BREAKMODEL);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_COORD(size.x);m.WRITE_COORD(size.y);m.WRITE_COORD(size.z);m.WRITE_COORD(velocity.x);m.WRITE_COORD(velocity.y);m.WRITE_COORD(velocity.z);m.WRITE_BYTE(speedNoise);m.WRITE_SHORT(g_engfuncs.pfnModelIndex(model));m.WRITE_BYTE(count);m.WRITE_BYTE(life);m.WRITE_BYTE(flags);m.MESSAGE_END(); }
...
void te_usertracer(Vector pos, Vector dir, float speed=6000.0f, uint8 life=32, uint color=4, uint8 length=12, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { Vector velocity = dir*speed;MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_USERTRACER);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_COORD(velocity.x);m.WRITE_COORD(velocity.y);m.WRITE_COORD(velocity.z);m.WRITE_BYTE(life);m.WRITE_BYTE(color);m.WRITE_BYTE(length);m.MESSAGE_END();}
...
void te_spritetrail(Vector start, Vector end, string sprite="sprites/hotglow.spr", uint8 count=2, uint8 life=0, uint8 scale=1, uint8 speed=16, uint8 speedNoise=8, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_SPRITETRAIL);m.WRITE_COORD(start.x);m.WRITE_COORD(start.y);m.WRITE_COORD(start.z);m.WRITE_COORD(end.x);m.WRITE_COORD(end.y);m.WRITE_COORD(end.z);m.WRITE_SHORT(g_engfuncs.pfnModelIndex(sprite));m.WRITE_BYTE(count);m.WRITE_BYTE(life);m.WRITE_BYTE(scale);m.WRITE_BYTE(speedNoise);m.WRITE_BYTE(speed);m.MESSAGE_END(); }
void te_streaksplash(Vector start, Vector dir, uint8 color=250, uint16 count=256, uint16 speed=2048, uint16 speedNoise=128, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_STREAK_SPLASH);m.WRITE_COORD(start.x);m.WRITE_COORD(start.y);m.WRITE_COORD(start.z);m.WRITE_COORD(dir.x);m.WRITE_COORD(dir.y);m.WRITE_COORD(dir.z);m.WRITE_BYTE(color);m.WRITE_SHORT(count);m.WRITE_SHORT(speed);m.WRITE_SHORT(speedNoise);m.MESSAGE_END(); }
void te_glowsprite(Vector pos, string sprite="sprites/glow01.spr", uint8 life=1, uint8 scale=10, uint8 alpha=255, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_GLOWSPRITE);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_SHORT(g_engfuncs.pfnModelIndex(sprite));m.WRITE_BYTE(life);m.WRITE_BYTE(scale);m.WRITE_BYTE(alpha);m.MESSAGE_END(); }
void te_bloodsprite(Vector pos, string sprite1="sprites/bloodspray.spr", string sprite2="sprites/blood.spr", uint8 color=70, uint8 scale=3, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_BLOODSPRITE);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_SHORT(g_engfuncs.pfnModelIndex(sprite1));m.WRITE_SHORT(g_engfuncs.pfnModelIndex(sprite2));m.WRITE_BYTE(color);m.WRITE_BYTE(scale);m.MESSAGE_END(); }
...
void _te_pointeffect(Vector pos, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL, int effect=TE_SPARKS) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(effect);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.MESSAGE_END(); }
...
void te_bubbletrail(Vector start, Vector end, string sprite="sprites/bubble.spr", float height=128.0f, uint8 count=16, float speed=16.0f, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_BUBBLETRAIL);m.WRITE_COORD(start.x);m.WRITE_COORD(start.y);m.WRITE_COORD(start.z);m.WRITE_COORD(end.x);m.WRITE_COORD(end.y);m.WRITE_COORD(end.z);m.WRITE_COORD(height);m.WRITE_SHORT(g_engfuncs.pfnModelIndex(sprite));m.WRITE_BYTE(count);m.WRITE_COORD(speed);m.MESSAGE_END(); }
void te_bubbles(Vector mins, Vector maxs, float height=256.0f, string sprite="sprites/bubble.spr", uint8 count=64, float speed=16.0f, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_BUBBLES);m.WRITE_COORD(mins.x);m.WRITE_COORD(mins.y);m.WRITE_COORD(mins.z);m.WRITE_COORD(maxs.x);m.WRITE_COORD(maxs.y);m.WRITE_COORD(maxs.z);m.WRITE_COORD(height);m.WRITE_SHORT(g_engfuncs.pfnModelIndex(sprite));m.WRITE_BYTE(count);m.WRITE_COORD(speed);m.MESSAGE_END(); }
void te_spritespray(Vector pos, Vector velocity, string sprite="sprites/bubble.spr", uint8 count=8, uint8 speed=16, uint8 noise=255, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_SPRITE_SPRAY);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_COORD(velocity.x);m.WRITE_COORD(velocity.y);m.WRITE_COORD(velocity.z);m.WRITE_SHORT(g_engfuncs.pfnModelIndex(sprite));m.WRITE_BYTE(count);m.WRITE_BYTE(speed);m.WRITE_BYTE(noise);m.MESSAGE_END(); }
void te_ricochet(Vector pos, uint8 scale=10, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_ARMOR_RICOCHET);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_BYTE(scale);m.MESSAGE_END(); }
...
void te_explosion2(Vector pos, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_EXPLOSION2);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_BYTE(0);m.WRITE_BYTE(127);m.MESSAGE_END();}
void te_particlebust(Vector pos, uint16 radius=128, uint8 color=250, uint8 life=5, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) {MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_PARTICLEBURST);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_SHORT(radius);m.WRITE_BYTE(color);m.WRITE_BYTE(life);m.MESSAGE_END();}
...
void te_implosion(Vector pos, uint8 radius=255, uint8 count=32, uint8 life=5, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL){MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_IMPLOSION);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_BYTE(radius);m.WRITE_BYTE(count);m.WRITE_BYTE(life);m.MESSAGE_END();}
void te_playersprites(CBasePlayer* target, string sprite="sprites/bubble.spr", uint8 count=16, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) {MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_PLAYERSPRITES);m.WRITE_SHORT(target->entindex());m.WRITE_SHORT(g_engfuncs.pfnModelIndex(sprite));m.WRITE_BYTE(count);m.WRITE_BYTE(0);m.MESSAGE_END();}
void te_bloodstream(Vector pos, Vector dir, uint8 color=70, uint8 speed=64, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) { MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_BLOODSTREAM);m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_COORD(dir.x);m.WRITE_COORD(dir.y);m.WRITE_COORD(dir.z);m.WRITE_BYTE(color);m.WRITE_BYTE(speed);m.MESSAGE_END();}
void te_elight(CBaseEntity* target, Vector pos, float radius=1024.0f, Color c=PURPLE, uint8 life=16, float decayRate=2000.0f, NetworkMessageDest msgType=MSG_BROADCAST, edict_t* dest=NULL) {MESSAGE_BEGIN m(msgType, SVC_TEMPENTITY, dest);m.WRITE_BYTE(TE_ELIGHT);m.WRITE_SHORT(target->entindex());m.WRITE_COORD(pos.x);m.WRITE_COORD(pos.y);m.WRITE_COORD(pos.z);m.WRITE_COORD(radius);m.WRITE_BYTE(c.r);m.WRITE_BYTE(c.g);m.WRITE_BYTE(c.b);m.WRITE_BYTE(life);m.WRITE_COORD(decayRate);m.MESSAGE_END();}
