...
{	
	PRECACHE_MODEL( g_watersplash_spr ); // used for water splash effect
...
void WeaponCustomMapActivate_internal()
{
	...
	vector<string>* keys = custom_weapons.getKeys();
	...
	vector<string>* keys2 = custom_weapon_shoots.getKeys();
...
int MAX_BEAMS = 256; // any more && you get console spam && game freezes (acutal max is 258 || 259 I think)
...
class weapon_custom_shoot : ScriptBaseEntity
{
	...
	int shoot_type;
	vector<WeaponSound> sounds; // shoot sounds
	vector<string> shoot_anims; // shoot || melee swing
	vector<string> melee_anims; // melee hit anims
	vector<WeaponSound> melee_hit_sounds;
	vector<WeaponSound> melee_flesh_sounds;
	vector<WeaponSound> shoot_fail_snds;
	...
	vector<BeamOptions> beams = {BeamOptions(), BeamOptions()};
	...
	{			
		...
		if (szKey == "sounds" || szKey == "melee_hit_sounds" || szKey == "melee_flesh_sounds" or
			szKey == "shoot_fail_snds" || szKey == "windup_snd" || szKey == "wind_down_snd" or
			szKey == "windup_loop_snd" || szKey == "hook_sound" || szKey == "hook_sound2" or
			szKey == "projectile_snd" || szKey == "shell_delay_snd" || szKey == "shoot_empty_snd")
		{
			if (g_map_activated)
				loadExternalSoundSettings();
		}
		
		if (szKey == "effect1_name" || szKey == "effect2_name" || szKey == "effect3_name" or
			szKey == "effect4_name" || szKey == "user_effect1" || szKey == "user_effect2" or
			szKey == "user_effect3" || szKey == "user_effect4" || szKey == "user_effect5" or
			szKey == "user_effect6")
		{
			if (g_map_activated)
	...
	WeaponSound* getRandomShootSound()
	{
		if (sounds.length() == 0)
			return NULL;
		int randIdx = RANDOM_LONG(0, sounds.length()-1);
	...
	WeaponSound* getRandomMeleeHitSound()
	{
		if (melee_hit_sounds.length() == 0)
			return WeaponSound();
		int randIdx = RANDOM_LONG(0, melee_hit_sounds.length()-1);
	...
	WeaponSound* getRandomMeleeFleshSound()
	{
		if (melee_flesh_sounds.length() == 0)
			return NULL;
		int randIdx = RANDOM_LONG(0, melee_flesh_sounds.length()-1);
	...
	WeaponSound* getRandomShootFailSound()
	{
		if (shoot_fail_snds.length() == 0)
			return NULL;
		int randIdx = RANDOM_LONG(0, shoot_fail_snds.length()-1);
	...
	void Spawn()
	{
		if (string(pev.targetname).Length() == 0) {
			println(logPrefix + "weapon_custom_shoot has no targetname && will not be used.");
			return;
		}
		...
		float bps = (1.0f / Math.max(0.01f, cooldown)) * iBullets;
	...
	void update_shell_type()
	{
		...
		if (shell_model.Length() > 0)
			shell_idx = PRECACHE_MODEL( shell_model );
	}
	...
	void Precache()
	{
		...
		if (projectile.entity_class.Length() > 0)
			UTIL_PrecacheOther( projectile.entity_class );
...
class weapon_custom : ScriptBaseEntity
{
	...
	// primary && secondary fire settings
	vector<weapon_custom_shoot*> fire_settings =
	{ weapon_custom_shoot(), weapon_custom_shoot(), weapon_custom_shoot() };
	
	vector<weapon_custom_shoot*> alt_fire_settings =
	{ weapon_custom_shoot(), weapon_custom_shoot(), weapon_custom_shoot() }; 
	...
	void update_active_weapons(string changedKey, string newValue)
	{
		...
		do {
			*ent = UTIL_FindEntityByClassname(ent, weapon_classname); 
			if (ent )
			{
	...
	{			
		...
		if (primary_fire.Length() == 0 && secondary_fire.Length() == 0)
		{
			println(logPrefix + weapon_classname + " has no primary || secondary fire function set");
			return;
		}
		...
		for (uint k = 0; k < keys2.length(); k++)
		{
			...
			if (shoot->pev->targetname == primary_fire && primary_fire.Length() > 0) {
				*fire_settings[0] = shoot;
				*shoot.weapon = this;
				foundPrimary = true;
			}
			if (shoot->pev->targetname == secondary_fire && secondary_fire.Length() > 0) {
				*fire_settings[1] = shoot;
				*shoot.weapon = this;
				foundSecondary = true;
			}
			if (shoot->pev->targetname == tertiary_fire && tertiary_fire.Length() > 0) {
				*fire_settings[2] = shoot;
				*shoot.weapon = this;
				foundTertiary = true;
			}
			if (shoot->pev->targetname == primary_alt_fire && primary_alt_fire.Length() > 0) {
				*alt_fire_settings[0] = shoot;
				*shoot.weapon = this;
				foundAltPrimary = true;
			}
		}
		if (!foundPrimary && primary_fire.Length() > 0)
			println(logPrefix + " Couldn't find primary fire entity " + primary_fire + " for " + weapon_classname);
		if (!foundSecondary && secondary_fire.Length() > 0)
			println(logPrefix + " Couldn't find secondary fire entity '" + secondary_fire + "' for " + weapon_classname);
		if (!foundTertiary && tertiary_fire.Length() > 0)
			println(logPrefix + " Couldn't find tertiary fire entity " + tertiary_fire + " for " + weapon_classname);
		if (!foundAltPrimary && primary_alt_fire.Length() > 0)
	...
	{
		if (player_anims < 0 || player_anims >= int(g_panim_refs.length()))
	...
	int getRandomIdleAnim()
	{
		if (idle_anims.length() == 0)
			return 0;
		int randIdx = RANDOM_LONG(0, idle_anims.length()-1);
	...
	{		
		...
		if (slotPosition == -1) // user chose "Auto"
		{
			...
			if (slotPosition == -1 || !isFreeWeaponSlot(slot, slotPosition))
				println(logPrefix + weapon_classname + " Can't fit in weapon slot " + slot +". Move this weapon to another slot && try again.");
		}
	...
	bool isFreeWeaponSlot(int slot, int position)
	{
		...
		vector<string>* stateKeys = custom_weapons.getKeys();
	...
	AmmoDrop getSmallestAmmoDropType(string ammoType)
	{
		...
		for (uint i = 0; i < keys.length(); i++)
		{
			...
			if (ammo.custom_ammo_type == ammoType && (ammo.give_ammo < best.dropAmt || best.dropAmt == -1))
			{
	...
	void Spawn()
	{
		if (weapon_classname.Length() > 0)
		{
			...
			if (primary_ammo_drop_amt == -1 && primary_ammo_drop_class.Length() > 0)
				println("WEAPON_CUSTOM ERROR: " + weapon_classname + " uses an invalid primary ammo drop class: " + primary_ammo_drop_class);	
			if (secondary_ammo_drop_amt == -1 && secondary_ammo_drop_class.Length() > 0)
			...
			if (primary_ammo_drop_class.Length() == 0 && primary_ammo_type.Length() > 0)
			{
			...
			if (secondary_ammo_drop_class.Length() == 0 && secondary_ammo_type.Length() > 0)
			{
				AmmoDrop bestMatch = getSmallestAmmoDropType(primary_ammo_type);
			...
			{
				UTIL_RegisterWeapon( weapon_classname, hud_sprite_folder, primary_ammo_type, "", 
					primary_ammo_drop_class, secondary_ammo_drop_class);
			}
			else
			{
				UTIL_RegisterWeapon( weapon_classname, hud_sprite_folder, primary_ammo_type, secondary_ammo_type,
					primary_ammo_drop_class, secondary_ammo_drop_class);
			}
...
{	
	...
	void Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float flValue = 0.0f)
	{
		...
		custom_effect(pev.origin, *this, h_ent, h_ent, h_ent, gpGlobals->v_forward, 0);
	}
};
